...
-- Split out from 20260123144521_river_tables.sql: this migration uses the
-- 'pending' river_job_state enum value in a function/index definition. It
-- must run as its own migration *file* (not just a later statement in the
-- same file) so the ADD VALUE from the previous migration is guaranteed to
-- be a fully separate, committed unit of work before this one starts --
-- Postgres forbids using a value added via ALTER TYPE ... ADD VALUE before
-- the transaction that added it commits, and some connection poolers/drivers
-- don't reliably autocommit each NoTransaction statement in isolation.
