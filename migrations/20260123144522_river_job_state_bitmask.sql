...
-- 'pending' river_job_state enum value in a function/index definition, which
-- must run in a migration *after* the one that added the enum value via
-- `ALTER TYPE river_job_state ADD VALUE 'pending'` (Postgres forbids using a
-- new enum value before the transaction that added it has committed).
